                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-start">
                    {result.status === 'success' ? (
                      ...
                      <Clock size={18} className="text-gray-500 mr-2 flex-shrink-0 mt-0.5 animate-pulse" />
                    )}
                    <div>
                      ...
                      {result.message && (
                        <p className={`text-sm mt-1 ${
                          darkMode 
                            ? result.status === 'success' ? 'text-green-300' : result.status === 'failure' ? 'text-red-300' : 'text-gray-400'
                            : result.status === 'success' ? 'text-green-700' : result.status === 'failure' ? 'text-red-700' : 'text-gray-500'
                        }`}>
                          {result.message}
                        </p>
                      )}
                      ...
                      {result.details && (
                        <div className={`mt-2 text-xs p-2 rounded ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
