...
import { SystemStats, Process, MLModel, ApiStats, WebsocketStats, StorageItem } from '../types';
...
export const stopProcess = async (pid: number): Promise<{ status: string; message: string }> => {
...
export const testModel = async (modelId: string): Promise<{ status: string; message: string; result?: any }> => {
...
// Storage/Cleanup
export const fetchLargeFiles = async (minSizeMB: number = 100, path: string = '/'): Promise<{ files: StorageItem[] }> => {
...
export const runCleanup = async (target: string): Promise<{ 
  status: string; 
  message: string; 
  details?: {
    bytesRemoved: number;
...
// Testing
export const testMongoDBConnection = async (uri?: string): Promise<{ 
  status: string; 
  message: string; 
  details?: {
    server: string;
